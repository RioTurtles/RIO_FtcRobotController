...
@TeleOp (name="tommyisOnTime")

public class Project5 extends LinearOpMode
{
    ...
    public void runOpMode() throws InterruptedException {
        ...
        double cur3 = timer3.milliseconds();
        double cur4 = timer4.milliseconds();
        ...
        boolean doublegrab = false;

        boolean horzreset = false;
        boolean vertreset = false;
        ...
        while (opModeIsActive()) {
            ...
            cur2 = timer2.milliseconds();
            cur3 = timer3.milliseconds();
            loops+=1;
           ...
           */
            ...
            if (stage == 0){
                ...
                if (!vertreset)robot.setVert(0);

                robot.yGuide.setPosition(0.2);


                if(!farming){
                    if ((gamepad1.right_bumper||clawcheck||gamepad1.dpad_right)&&clawDown){
                        ...
                        sleep(200);
                        horzTarget=0;
                        if (!horzreset)robot.setHorz(horzTarget);
                        toggleTimer=0;
                        timer2.reset();
                        stage += 1;
                    ...
                    robot.setArm(0.05);
                    robot.clawAngle.setPosition(0.205);
                    if (!horzreset)robot.setHorz(farmingHorz);
            ...
            if (stage==1){
                if(!ground){
                    clawDown=false;
                    if ((((robot.lHorz.getCurrentPosition()<500)||(robot.lHorz.getCurrentPosition()<1800&&robot.lHorz.getCurrentPosition()>1000))&&(cur2>250))&&(!loaded)){
                        robot.setArm(0.85);
                    ...
                    if(gamepad1.right_bumper){
                        robot.clawAngle.setPosition(0.205);//horizontal
                        robot.setArm(0.2);//ground level
                        groundScoring = true;

                    }
                    if(groundScoring&&!gamepad1.right_bumper&&cur2>250){
                        drivetrain.remote(0,0,0,0);
                        robot.clawOpen();
                        sleep(500);
                        groundScoring=false;
            ...
            if (stage ==2){
                if(!loaded){
                    if (cur2 > 150){
                        robot.clawAngle.setPosition(0.78);
            ...
            if (stage ==3){
                ...
                if(robot.lVert.getCurrentPosition()>20){
                    timer2.reset();
                }
                if (cur2 > 150) {
            ...
            if (loaded){
                ...
                if (gamepad1.right_bumper){
                    if (!vertreset)robot.setVert(vertTarget+vertadd);
                ...
                if (farming){
                    ...
                    robot.setArm(0.05);
                    robot.clawAngle.setPosition(0.205);
                    if (!horzreset)robot.setHorz(farmingHorz-800);
                    lowcontrol=false;
            ...
            if(!farming){
                if (gamepad1.left_bumper){
                    ...
                    robot.clawAngle.setPosition(0.205);//horizontal position
                    clawDown = true;
                    if((cur3>150)&&(robot.lHorz.getCurrentPosition()<2000)){
            ...
            if(gamepad1.touchpad&&cur4>1000){
                doublegrab=!doublegrab;
                stage=0;
                clawDown=true;
                robot.clawAngle.setPosition(0.205);
            ...
            if (gamepad1.share){
                horzreset = true;
                robot.lHorz.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
                robot.lHorz.setPower(-1);
                robot.rHorz.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
                robot.rHorz.setPower(-1);
            }

            if (!gamepad1.share&&horzreset){
                robot.lHorz.setPower(0);
                robot.rHorz.setPower(0);
                horzreset = false;
                if (!horzreset)robot.setHorz(0);
            ...
            if (gamepad1.options){
                vertreset = true;
                robot.lVert.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
                robot.lVert.setPower(-1);
                robot.rVert.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
                robot.rVert.setPower(-1);
            }

            if (!gamepad1.options&&horzreset){
                robot.lVert.setPower(0);
                robot.rVert.setPower(0);
                vertreset = false;
                if (!vertreset)robot.setVert(0);
            }
